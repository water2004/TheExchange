...
public final class PlayerWarehouseHopperBridge {
    ...
    public static Optional<Boolean> push(Level level, BlockPos hopperPos, HopperBlockEntity hopper) {
        ...
        if (session.isEmpty()) {
            session = PlayerWarehouseAutomationSessions.find(
    ...
    private static CompletableFuture<ExchangeMutationResult> takeFirstAcceptable(
        ...
        var source = WarehouseAutomationPlanner.findTakeSlot(
                state.getItems(), item -> canAccept(core, destinationSnapshot, item));
        if (source.isEmpty()) {
            return CompletableFuture.failedFuture(
                    new IllegalStateException("玩家仓库没有漏斗可接收的兼容物品"));
        }
        return core.takeRemoteAsync(connection.serverName(), source.getAsInt(), 1, actor, access);
    }

    private static CompletableFuture<InventoryAccess> access(
            TheExchangeCore core, PlayerInventoryConnectionSpec connection,
            PlayerExchangeContext actor, EndpointKey endpointKey,
            Optional<InventoryAccess> existing) {
        ...
        return core.authenticatePlayerInventoryAsync(connection.serverName(), connection.playerName(),
                ...
                .whenComplete((ignored, error) -> {
                    if (error == null) {
                        AUTH_RETRY_AFTER.remove(endpointKey);
                    } else {
                        AUTH_RETRY_AFTER.put(endpointKey,
                                System.currentTimeMillis() + AUTH_FAILURE_BACKOFF_MS);
                    }
                });
    }

    private static CompletableFuture<ExchangeViewState> open(
        ...
        return "local".equalsIgnoreCase(connection.serverName())
                || localName.equalsIgnoreCase(connection.serverName())
    ...
    private static void insertPulled(Level level, Hopper hopper, ItemStack stack) {
        ...
        if (hopper instanceof Entity entity) {
            available = entity.isAlive();
        }
        ItemStack remaining = available
                ? HopperBlockEntity.addItem(null, hopper, stack.copy(), null) : stack.copy();
        if (!remaining.isEmpty()) {
            Containers.dropItemStack(level, hopper.getLevelX(), hopper.getLevelY(),
                    hopper.getLevelZ(), remaining);
        }
    }
    ...
    private static PlayerExchangeContext automationActor(Level level, BlockPos hopperPos) {
        ...
        String uuid = UUID.nameUUIDFromBytes(("theexchange:hopper:" + endpoint)
    ...
    private static PlayerExchangeContext automationActor(Level level, BlockPos hopperPos, Hopper hopper) {
        if (hopper instanceof Entity entity) {
            return new PlayerExchangeContext(entity.getUUID().toString(), "Hopper Minecart");
        }
    ...
    private static boolean authBackoffActive(EndpointKey key) {
        ...
        if (retryAfter <= System.currentTimeMillis()) {
            AUTH_RETRY_AFTER.remove(key, retryAfter);
            return false;
        }
        return true;
    }
