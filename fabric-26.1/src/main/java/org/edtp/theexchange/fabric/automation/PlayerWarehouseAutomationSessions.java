...
public final class PlayerWarehouseAutomationSessions {
    ...
    public static void remember(String endpointId, PlayerInventoryConnectionSpec connection, InventoryAccess access) {
        if (endpointId == null || endpointId.isBlank() || access == null || !access.isPlayer()) return;
        SESSIONS.put(endpointId, new DelegatedSession(normalize(connection.serverName()), normalize(connection.playerName()), access));
    }
    ...
    public static Optional<InventoryAccess> find(TheExchangeCore core, String endpointId,
        ...
        if (delegated == null || !delegated.serverName().equals(normalize(connection.serverName()))
                || !delegated.ownerName().equals(normalize(connection.playerName()))) return Optional.empty();
        InventoryAccess descriptor = delegated.access();
        Optional<InventoryAccess> current = core.findPlayerInventorySession(connection.serverName(), connection.playerName(),
